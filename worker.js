  ...
  <script>
    ...
    if (request.method === "POST") {
      try {
        ...
        // Perbarui prompt dengan instruksi yang lebih jelas
        const payload = {
          prompt: `
          Analyze the provided image and generate:
1. A descriptive, specific, SEO-friendly title between 10 and 15 words. The title should be engaging and keyword-rich.
2. A list of exactly 30 unique, relevant English keywords or short phrases related to the image. Each keyword must be concise (maximum 2 words) and unique. The keywords must be separated by commas.
...
Do not include any additional explanations or text.
        ...
        // Fungsi untuk memanggil API AI Cloudflare
        ...
        // Fungsi untuk memeriksa apakah output AI valid (title ada dan menghilangkan keyword duplikat)
        async function checkAIOkay(imageArray) {
          ...
          while (retries < maxRetries) {
            try {
              ...
              // Ekstrak title dan keywords menggunakan regex
              const titleMatch = aiResponse.match(/title:\s*(.*?)\s*keywords:/i);
              const keywordMatch = aiResponse.match(/keywords:\s*(.*)/i);

              const title = titleMatch ? titleMatch[1].trim() : "";
              let keywordsStr = keywordMatch ? keywordMatch[1].trim() : "";
              let keywordsArr = keywordsStr.split(',')
                .map(k => k.trim().toLowerCase())
                .filter(k => k !== "");

              // Hilangkan keyword duplikat
              keywordsArr = [...new Set(keywordsArr)];

              // Validasi: pastikan title ada
              if (!title) {
                console.error("No title generated. Retrying...");
                retries++;
                continue;
              }

              return { title, keywords: keywordsArr.join(', ') };
            } catch (error) {
              retries++;
              console.error(`AI call failed, retrying... (${retries}/${maxRetries})`, error);
